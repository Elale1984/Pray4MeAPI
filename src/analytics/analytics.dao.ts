...
export const deleteAnalytics = async (userId: number) => {
    return execute<OkPacket>(analyticsQueries.deleteAnalytics, [userId]);
}

export const updateAnalytics = async (analytics: Analytics) => {
    return execute<OkPacket>(analyticsQueries.updateAnalytics, 
        [analytics.prayerRequestCount, analytics.prayerCount, analytics.answeredCount, analytics.userId]);
}
